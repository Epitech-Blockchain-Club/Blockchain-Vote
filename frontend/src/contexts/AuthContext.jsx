...
export function AuthProvider({ children }) {
  ...
  const fetchOAuthConfig = async () => {
    try {
      const res = await fetch(`${API_ROUTES.AUTH}/oauth-config`)
    ...
    return new Promise((resolve) => {
      ...
      const messageHandler = async (event) => {
        if (event.data.type === 'oauth-voter-success' || event.data.type === 'oauth-success') {
          ...
          try {
            const res = await fetch(`${API_ROUTES.AUTH}/oauth-login`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
  ...
  const loginWithToken = async (token) => {
    try {
      const res = await fetch(`${API_ROUTES.AUTH}/moderator/verify`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const verifyVoterForScrutin = async (email, scrutinId) => {
    try {
      const res = await fetch(`${API_ROUTES.AUTH}/verify-voter`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const getAvailableScrutins = async (email) => {
    try {
      const res = await fetch(`${API_ROUTES.SCRUTINS}/available?email=${encodeURIComponent(email)}`)
      const result = await res.json()
